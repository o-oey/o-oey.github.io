const MS = {
  fallback:
    'https://images.unsplash.com/photo-1514282401047-d79a71a590e8?auto=format&fit=crop&w=1400&q=70',
  money(n) {
    if (n == null || n === 0) return 'On request';
    return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(n);
  },
  img(url) {
    return url || MS.fallback;
  },
  ...
  header(active) {
    ...
    el.innerHTML = `
      <a class="ms-brand" href="/">
        <img src="/assets/logo-official.svg" alt="Maldives Sales official logo" onerror="this.src='/assets/logo-mark.svg'">
        <span class="word">
          <strong>Maldives Sales</strong>
  ...
  footer() {
    ...
    el.innerHTML = `
      <div class="wrap">
        <div>
          <img src="/assets/logo-official.svg" alt="Maldives Sales" onerror="this.src='/assets/logo-mark.svg'">
          <p style="margin:12px 0 0;max-width:36ch">Maldives Sales is a discovery and concierge desk for the archipelago — 20 atolls, private islands, resorts and local guesthouses, with live inventory logic ready for production booking.</p>
        </div>
        <div>
          <h4>Explore</h4>
          <p><a href="/atolls.html">Atolls</a></p>
          <p><a href="/explorer.html">All stays</a></p>
        ...
        <div>
          <h4>Deploy</h4>
          <p>Node API or static</p>
          <p>Inquiry ledger</p>
          <p>Docker ready</p>
        </div>
        <p class="legal">© ${new Date().getFullYear()} Maldives Sales. Independent platform inspired by the public Maldives Agent catalogue, rebuilt and rebranded for Maldives Sales. Rates shown are indicative and confirmed by the desk.</p>
      </div>`;
  },
  mountSearch() {
    ...
    wrap.innerHTML = `
      <div class="search-box">
        <input id="searchInput" placeholder="Search a resort, island or atoll" autofocus>
  ...
  async onSearch(e) {
    ...
    box.innerHTML = [
      ...
      ...data.properties.map((p) => `<a class="search-hit" href="/property.html?id=${encodeURIComponent(p.id)}"><img src="${MS.img(p.image)}" alt=""><div><strong>${p.name}</strong><small>${p.island || p.atoll} · ${p.tier} · from ${MS.money(p.priceFrom)}</small></div></a>`),
    ].join('') || '<p class="muted">No matches.</p>';
  },
  card(p) {
    return `<a class="card" href="/property.html?id=${encodeURIComponent(p.id)}">
      <div class="card-media">
        <img src="${MS.img(p.image)}" alt="${p.name}" loading="lazy" onerror="this.src='${MS.fallback}'">
        <span class="badge">${p.tier}</span>
      </div>
      <div class="card-body">
        <h3>${p.name}</h3>
        <p class="meta">${p.island || 'Private island'} · ${p.atoll}</p>
        <p class="price">${MS.money(p.priceFrom)} <em>/ night</em></p>
      </div>
    </a>`;
  },
};
